  ...
  function initBookingForm() {
    ...
    form.addEventListener('submit', async function(event) {
      ...
      const templateParams = {
        from_name: [firstName, lastName].filter(Boolean).join(' '),
        reply_to: email,
      ...
      try {
        ...
        if (status) {
          status.textContent = response?.text || 'Booking request sent successfully.';
        }
      } catch (error) {
        const errorMessage = error?.text || error?.message || 'Unknown EmailJS error';
        const lowerMessage = String(errorMessage).toLowerCase();
        const gmailGrantIssue = lowerMessage.includes('invalid grant') || lowerMessage.includes('gmail_api');
        console.error('EmailJS booking form failed:', errorMessage, error);
        if (status) {
          status.textContent = gmailGrantIssue
            ? 'Email service needs to be reconnected in EmailJS: Gmail API invalid grant. Reconnect your Gmail account and try again.'
            : `Something went wrong sending the form: ${errorMessage}`;
        }
      } finally {
        if (submitButton) {
