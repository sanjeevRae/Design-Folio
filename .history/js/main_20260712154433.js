...
(function() {
  ...
  function initBookingForm() {
    ...
    const fromNameField = document.getElementById('fromNameField');
    ...
    form.addEventListener('submit', async function(event) {
      ...
      if (fromNameField) {
        fromNameField.value = [firstName, lastName].filter(Boolean).join(' ');
      }
      ...
      try {
        ...
        if (status) {
          status.textContent = `Something went wrong sending the form: ${errorMessage}`;
        }
      } finally {
        if (submitButton) {
